...
export default function ChatRoom({ user: initialUser, token, onLogout, onUpdateUser }) {
  ...
  const roomLoadTimer = useRef(null)
  ...
  useEffect(() => {
    ...
    function onHistory(history) {
      ...
      clearTimeout(roomLoadTimer.current)
    ...
    function onRoomList(list, counts, grps) { setRooms(list); roomsRef.current = list; if (counts) setRoomCounts(counts); if (grps) setGroups(grps) }
    function onRoomJoined({ roomId }) {
      setCurrentRoom(roomId)
      // If this room is not in our list, our list is stale. Refresh it so the
      // header and sidebar always reflect reality.
      if (!roomsRef.current.some(r => r.id === roomId)) socket.emit('getRooms')
      setMessages([])          // history follows immediately after
      setRoomLoading(true)     // show a placeholder instead of a blank screen
      // Tell the user where they landed. Without this the switch is silent and
      // an empty room looks like the app broke.
      const r = roomsRef.current.find(x => x.id === roomId)
      if (r) {
        const nm = (langRef.current === 'en' && r.nameEn) ? r.nameEn : r.name
        showNotif(`${r.emoji} ${nm}`)
      }
      // Never leave the room stuck on "loading" if history is slow or dropped
      clearTimeout(roomLoadTimer.current)
      roomLoadTimer.current = setTimeout(() => setRoomLoading(false), 4000)
    }
  ...
  // Resolve the header from the room we are ACTUALLY in. If the room list has
  // not arrived yet we must not claim to be somewhere else: showing a hardcoded
  // "Main" here made the header lie whenever the client had a stale room list.
    ...
    <div className="chat-container" onClick={() => { setContextMenu(null); setShowEmoji(false) }}>
      ...
      <div className="chat-main">
        {showRooms && (
          <div className="rooms-panel" onClick={e => e.stopPropagation()}>
            ...
            <div className="rooms-list">
              ...
              {/* Safety net: any room whose group we don't know about is still
                  listed, so a room can never silently vanish from the sidebar. */}
              {rooms.filter(r => !groups.some(g => g.id === r.group)).map(room => (
                <button
                  key={room.id}
                  className={`room-item ${room.id === currentRoom ? 'room-active' : ''}`}
                  onClick={() => switchRoom(room.id)}
        ...
        <div className="messages-area" ref={messagesAreaRef} onScroll={handleScroll}>
          {messages.length === 0 && (
            <div className="empty-chat">{roomLoading ? L.connectingShort : L.emptyChat}</div>
          )}
          {messages.map((msg, i) => (
            <Message
              key={msg.id || i}
